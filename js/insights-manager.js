    ...
    renderInsights() {
        ...
        container.innerHTML = roleInsights.map(item => {
            ...
            if (this.currentMetrics) {
                ...
                // 3. Operator Analysis (Custom Stability Logic - Prioritize Latency & Upload)
                if (item.category === "Operator Analysis") {
                    // Stable: Good Upload (>15) and Low Latency (<45)
                    // Adjusted per user request to focus on "real time latency and upload speed"
                    if (lat < 45 && up > 15) {
                        item.trend = "stable";
                        item.insight = `[Stable] Network metrics for ${location} indicate solid real-time performance (Low Latency: ${Math.round(lat)}ms, Upload: ${up.toFixed(1)} Mbps).`;
                    }
                    // Unstable: High Latency (>100) OR Very Low Upload (<2)
                    else if (lat > 100 || up < 2) {
                        item.trend = "unstable";
                        item.insight = `[Unstable] High latency (${Math.round(lat)}ms) or poor upload speed detected in ${location}.`;
                    }
                    // Neutral: Everything else
                    else {
                        item.trend = "neutral";
                        item.insight = `[Neutral] Connectivity in ${location} is adequate but shows average response times.`;
                    }
                }
